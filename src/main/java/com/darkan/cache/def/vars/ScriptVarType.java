...
public enum ScriptVarType {
	...
	private static ScriptVarType[] legacyMappings = new ScriptVarType[256];
	private static Map<Integer, ScriptVarType> BY_ID = new HashMap<>();
	private static Map<Character, ScriptVarType> BY_CHAR = new HashMap<>();

	static {
		for (ScriptVarType t : ScriptVarType.values()) {
			legacyMappings[Utils.charToCp1252(t.c) & 0xff] = t;
			BY_ID.put(t.id, t);
			BY_CHAR.put(t.c, t);
		}
	}
	...
	private char c;
	private BaseVarType type;
	private Object defaultVal;
	
	private ScriptVarType(int id, String display, char c, BaseVarType type, Object defaultVal) {
		this.id = id;
	...
	public Object rawToReadable(Object id) {
        if (id instanceof String && type == BaseVarType.STRING) 
        	return id;
        else if (id instanceof String) 
        	throw new IllegalArgumentException("Raw type is string, but base type is " + type);
